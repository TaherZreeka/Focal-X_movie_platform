...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <section class="content">
        <form method="POST" action="{{ route('content_admin.movies.update', $movie->id)  }}"  enctype="multipart/form-data">
                    ...
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="description" name="description" class="form-control" value="{{ old('description',$movie->description) }}"
                                    required>
                            </div>
                             <div class="form-group">
                                <label for="poster_url">poster</label>
                                <input type="url" id="poster" name="poster_url"accept="image/*" class="form-control" value="{{ old('poster_url',$movie->poster_url) }}"
                                    required>
                            </div>
                            <div class="form-group">
                                <label for="trailer_url">trailer</label>
                                <input type="url" id="trailer_url" name="trailer_url"accept="video/*" class="form-control" value="{{ old('trailer_url',$movie->trailer_url) }}"
                                    required>
                            </div>

                            <div class="form-group">
                                <label for="movie_url">movie</label>
                                <input type="file" id="movie_url" name="movie_url"accept="video/*" class="form-control" value="{{ old('movie_url',$movie->movie_url) }}"
                                    required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <select id="language" name="language" class="form-control custom-select" required>
                                    <option disabled {{ is_null($movie->language) ? 'selected' : '' }}>Select one</option>
                                    <option value="Arabic" {{ $movie->language === 'Arabic' ? 'selected' : '' }}>Arabic</option>
                                    <option value="English" {{ $movie->language === 'English' ? 'selected' : '' }}>English</option>
                                    <option value="Franch" {{ $movie->language === 'Franch' ? 'selected' : '' }}>Franch</option>
                            ...
                            <div class="form-group">
                                ...
                                <select id="age_rating" name="age_rating" class="form-control custom-select" required>
                                    <option disabled {{ is_null($movie->age_rating) ? 'selected' : '' }}>Select one</option>
                             ...
                             <select name="genres[]" multiple required class="w-full border rounded px-3 py-2">
                                     @foreach($genres as $genre)
                                        <option value="{{ $genre->id }}"
                                          {{ in_array($genre->id,$selectGenres ) ? 'selected' : '' }}>
                                                   {{ $genre->name }}
                                         </option>
                                      @endforeach
                                           </select>
